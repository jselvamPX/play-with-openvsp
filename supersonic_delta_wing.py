...
class SupersonicDeltaWingTest:
    ...
    def __init__(self):
        ...
        self.Cl_alpha_tan_sweep = [
            [0.0] * len(self.m_SuperMachVec) for i in range(len(self.m_Sweep))
        ]
        self.M_sweep_fun = [
            [0.0] * len(self.m_SuperMachVec) for i in range(len(self.m_Sweep))
        ]
    ...
    def GenerateSupersonicDeltaWing(self):
        ...
        for s in self.m_Sweep:
            ...
            for im, m in enumerate(self.m_SuperMachVec):
                ...
                fname = (
                    scriptpath
                    + "/supersonic_files/vsp_files/Supersonic_Delta_Wing_Sweep"
                    + str(s)
                    + "_Mach"
                    + str(im)
                    + ".vsp3"
        ...
        for s in range(num_sweep):
            for m in range(num_mach):
                ...
                vsp.SetIntAnalysisInput(
                    const.m_CompGeomAnalysis, "GeomSet", [vsp.SET_NONE], 0
                )
                vsp.SetIntAnalysisInput(
                    const.m_CompGeomAnalysis, "ThinGeomSet", [vsp.SET_ALL], 0
                )  # Thin geometry - VLM
                vsp.SetIntAnalysisInput(
                    const.m_CompGeomAnalysis, "Symmetry", const.m_SymFlagVec, 0
                )
                ...
                vsp.SetIntAnalysisInput(
                    const.m_VSPSweepAnalysis, "GeomSet", [vsp.SET_NONE], 0
                )
                vsp.SetIntAnalysisInput(
                    const.m_VSPSweepAnalysis, "ThinGeomSet", [vsp.SET_ALL], 0
                )  # Thin geometry - VLM
                vsp.SetIntAnalysisInput(
                    const.m_VSPSweepAnalysis, "RefFlag", const.m_RefFlagVec, 0
                )
                vsp.SetIntAnalysisInput(
                    const.m_VSPSweepAnalysis, "Symmetry", const.m_SymFlagVec, 0
                )
                ...
                vsp.SetDoubleAnalysisInput(
                    const.m_VSPSweepAnalysis, "AlphaStart", Alpha, 0
                )
                AlphaNpts = [1]
                vsp.SetIntAnalysisInput(
                    const.m_VSPSweepAnalysis, "AlphaNpts", AlphaNpts, 0
                )
                vsp.SetDoubleAnalysisInput(
                    const.m_VSPSweepAnalysis, "MachStart", Mach, 0
                )
                MachNpts = [1]
                vsp.SetIntAnalysisInput(
                    const.m_VSPSweepAnalysis, "MachNpts", MachNpts, 0
                )
                vsp.SetIntAnalysisInput(
                    const.m_VSPSweepAnalysis, "WakeNumIter", const.m_WakeIterVec, 0
                )
                ...
                if len(rid_vec) > 0:
                    ...
                    self.Cl_alpha_tan_sweep[s][m] = Cl_alpha_vsp * math.tan(
                        math.radians(self.m_Sweep[s])
                    )
                    self.M_sweep_fun[s][m] = math.sqrt(
                        (math.pow(self.m_SuperMachVec[m], 2)) - 1
                    ) / math.tan(math.radians(self.m_Sweep[s]))
                vsp.ClearVSPModel()
    ...
    def GenerateSupersonicDeltaWingCharts(self):
        ...
        fig.savefig(
            scriptpath
            + "/supersonic_files/supersonic_img/supersonic/geometrysetup.svg",
            bbox_inches="tight",
        )
        ...
        data = [
            ...
            ["5.0"],
            ["0.0"],
            ["1.135,1.366,1.894,2.386,2.861,3.369,3.884,4.404"],
            ["3"],
        ]
        ax.table(cellText=const.transpose(data), colLabels=header, loc="upper center")
        fig.savefig(
            scriptpath + "/supersonic_files/supersonic_img/supersonic/vspaerosetup.svg",
            bbox_inches="tight",
        )
        ...
        for a in range(len(self.m_Sweep)):
            ax.plot(
                self.M_sweep_fun[a],
                self.Cl_alpha_tan_sweep[a],
                "o-",
                color=const.colors[a],
                label="VSPAERO " + str(self.m_Sweep[a]) + "° Sweep",
            )
        ax.scatter(
            self.m_exp,
            self.Cl_alpha_tan_sweep_exp,
        ...
        fig.savefig(
            scriptpath + "/supersonic_files/supersonic_img/supersonic/only.svg",
            bbox_inches="tight",
        )
    ...
    for i in range(len(testnames)):
        for j in range(len(subnames[i])):
            for k in range(len(subsubnames[i][j])):
                dirname = Path.joinpath(
                    scriptpathlib, testnames[i] + subnames[i][j] + subsubnames[i][j][k]
                )
